...
export default function ProjectsArchive({ openProject, navigateTo }) {
  // Exclude featured projects for general listing or show all
  const allProjects = Object.values(PROJECTS_DB);
        ...
        <div className="max-w-6xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {allProjects.map(project => (
                    <div key={project.id} onClick={() => openProject(project.id)} className="project-card p-8 bg-zinc-900/50 border border-zinc-800 rounded-[2rem]">
            ...
            <div className="mt-20 text-center">
                <button onClick={() => navigateTo('home')} className="px-10 py-4 border border-zinc-800 rounded-2xl hover:bg-zinc-900 transition font-bold text-sm tracking-widest uppercase">
                    Back to Home
                </button>
            </div>
        </div>
